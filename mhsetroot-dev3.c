...
void usage (char *commandline)
{
  printf (PACKAGE_STRING " - " DESCRIPTION "\n"
        ...
        "\n"
        "Dia:\n"
        " -Dia <Img-diamentions>     Renders to user spects within monitor\n"
        "                            abilities / settings \n"
        "                            format  100x100\n"
        "Solid:\n"
...
typedef enum
{ Full, Fill, Center, Tile, Dia, Tilex } ImageMode;
...
int load_Mod_image (ImageMode mode, const char *arg, int rootW, int rootH,
				int alpha, Imlib_Image rootimg)
{
	printf("in mod fun rootW %d rootH%d\n,",rootW,rootH);
	int left, top, x, y;
	int width, height; 
	int imgW, imgH, o;
	...
	if (mode == Tilex)
	{
		printf("in tilex function rootW%d rootH%d imgW%d imgH%d\n", rootW,rootH,imgW,imgH);
		left = (width - rootW) / 2;
			top = (height - rootH) /2;
...
int main (int argc, char **argv)
{
	...
	// end of new tiliing setting -tilex
