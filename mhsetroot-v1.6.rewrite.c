...
typedef enum
{ Fill, Full, Center, Tile, Dimg, DTile, DTileH, DTileV, DTileHV,
	DFlipimgH, DFlipimgV, DFlipimgD, OTilev, OTileh, OTilet } ImageMode;
*/
...
void usage  (char *commandline)
{
   ...
   "\n"
	...
	"Example:\n"
	" mhsetroot -addd ""\"""#FF0000""\" 3 -addd ""\"""#0000FF""\" 5 -gradient 45 -alpha 50 -dflipimgD 100x200 <PathToImage/ImageName.ext>\n"
	"Misc:\n"
...
int setOrginalSizeImageTiled_within_itself(ImageMode mode, const char *arg, int rootW, int rootH, int alpha, Imlib_Image rootimg) {		
	int left, top, x, y;
	int width, height; 
	int imgW, imgH;
...
int main (int argc, char **argv)
{
	...
	else if (strcmp (argv[i], "-tileH") == 0)
	{
		if ((++i) >= argc)
		{
			...
			if (setOrginalSizeImageTiled_within_itself(TileH, argv[i], width, height, alpha, image) == 1)
			{
				fprintf (stderr, "Bad image (%s)\n", argv[i]);
				continue;
			}
	...
	else if (strcmp (argv[i], "-tileV") == 0)
	{
		if ((++i) >= argc)
		{
			...
			if (setOrginalSizeImageTiled_within_itself(TileV, argv[i], width, height, alpha, image) == 1)
			{
				fprintf (stderr, "Bad image (%s)\n", argv[i]);
				continue;
			}
	...
	else if (strcmp (argv[i], "-tileHV") == 0)
	{
		if ((++i) >= argc)
		{
			...
			if (setOrginalSizeImageTiled_within_itself(TileHV, argv[i], width, height, alpha, image) == 1)
			{
				fprintf (stderr, "Bad image (%s)\n", argv[i]);
				continue;
			}
